...
//include 'koneksi.php';
include 'fungsi.php';
//mysqli_query("TRUNCATE pohon_keputusan");
...
function pangkas($PARENT, $KASUS, $LEAF) {
    $sql_pangkas = mysqli_query("SELECT * FROM pohon_keputusan WHERE parent=\"$PARENT\" AND keputusan=\"$LEAF\"");
    ...
    if ($jml_pangkas == 0) {
        mysqli_query("INSERT INTO pohon_keputusan (parent,akar,keputusan)VALUES (\"$PARENT\" , \"$KASUS\" , \"$LEAF\")");
    }
    //jika keputusan dan parent sudah ada maka delete
    else {
        mysqli_query("DELETE FROM pohon_keputusan WHERE id='$row_pangkas[0]'");
        ...
        $que_pangkas = mysqli_query("SELECT * FROM pohon_keputusan WHERE parent=\"$imPangkas\" AND keputusan=\"$LEAF\"");
        $baris_pangkas = mysqli_fetch_array($que_pangkas);
        $jumlah_pangkas = mysqli_num_rows($que_pangkas);

        if ($jumlah_pangkas == 0) {
            mysqli_query("INSERT INTO pohon_keputusan (parent,akar,keputusan)VALUES (\"$imPangkas\" , \"$akarPangkas\" , \"$LEAF\")");
            //mysqli_query("UPDATE pohon_keputusan SET parent=\"$imPangkas\" , akar=\"$akarPangkas\" , keputusan=\"$LEAF\" WHERE id=\"$row_pangkas[0]\"");
        } else {
...
function pembentukan_tree($N_parent, $kasus) {
    ...
    if ($cek == 'homogen') {
        echo "<br>LEAF ";
        $sql_keputusan = mysqli_query("SELECT DISTINCT(ipk) FROM data_training WHERE $kondisi");
        ...
        //lakukan perhitungan
            ...
            $sql_max = mysqli_query("SELECT MAX(gain) FROM gain");
            $row_max = mysqli_fetch_array($sql_max);
            $max_gain = $row_max['0'];
            $sql = mysqli_query("SELECT * FROM gain WHERE gain=$max_gain");
