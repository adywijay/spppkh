...
class Mining extends CI_Controller { 
    ...
    function pangkas($PARENT, $KASUS, $LEAF){
		...
		$row_pangkas = $sql_pangkas->result();
		...
		//jika keputusan dan parent sudah ada maka delete
			...
			$this->db->query("DELETE FROM tbl_keputusan WHERE id='$row_pangkas[0]'");
			...
			$baris_pangkas = $que_pangkas->result();
	...
	function pembentukan_tree($N_parent , $kasus){
		...
		if($cek=='homogen'){
			...
			$row_keputusan = $sql_keputusan->result();	
			...
			//cek jumlah data
			$jumlah = $this->jumlah_data($kondisi);				
			if($jumlah<8){
				echo "<br>LEAF ";
				$Nya = $kondisi."keputusan_asli='YA'";
				$NTidak = $kondisi."keputusan_asli='TIDAK'";
				$jumlahYA = $this->jumlah_data("$Nya");
				$jumlahTIDAK = $this->jumlah_data("$NTidak");				
				if($jumlahYA <= $jumlahTIDAK){
					$keputusan = 'TIDAK';
				}else{
					$keputusan = 'YA';
				}				
				//insert atau lakukan pemangkasan ca
				$this->pangkas($N_parent , $kasus , $keputusan);
			}
			//lakukan perhitungan
            else{			
                    ...
                    $nilai_sta_asuransi_lain = $this->cek_nilaiAtribut('asuransi_lain',$kondisi);
...
//-------------------------------------------------------------------------------------------------------> hitung gain atribut
                    $this->db->query("TRUNCATE tbl_gain");
                     ...
                     if($jmlKeluarga!=1){//--------------------------------------- Jumlah Keluarga
                        ...
                        $this->hitung_gain($kondisi, "status", $entropy_all, $NA1Jkga, $NA2Jkga, "", "", "");
                     }
	...
	public function jumlah_data($kondisi){
		//sql
		if($kondisi==''){
			$sql = $this->db->query("SELECT COUNT(*) FROM tbl_data_latih $kondisi");	
		}else{
			$sql = $this->db->query("SELECT COUNT(*) FROM tbl_data_latih WHERE $kondisi");						
		}		
		$row = $sql->result();	
		$jml = $row['0'];
		return $jml;
	...
	public function hitung_gain($kasus , $atribut , $ent_all , $kondisi1 , $kondisi2 , $kondisi3 , $kondisi4 , $kondisi5, $kondisi6, $kondisi7, $kondisi8, $kondisi9, $kondisi10, $kondisi11, $kondisi12){
		...
		if($kondisi3==''){
			$j_YA1 = $this->$this->jumlah_data("$data_kasus keputusan_asli='YA' AND $kondisi1");
		...
		$this->db->query("INSERT INTO tbl_gain VALUES ('','$atribut','$gain')");
	}
	...
	public function hitung_rasio($kasus , $atribut , $gain , $nilai1 , $nilai2 , $nilai3 , $nilai4 , $nilai5){				
		...
		$sql_max = $this->db->query("SELECT MAX(rasio_gain) FROM rasio_gain");
		$row_max = $sql_max->result();	
		$max_rasio = $row_max['0'];
